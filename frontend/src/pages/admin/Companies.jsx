...
import { getAllCompanies , deleteCompany } from '../../API/Req'
...
const Companies = () => {
  ...
  function getCompaniesThenFetch(){
    try {
  ...
  const DeleteCompanyById = (id) => {
    try {
      deleteCompany(id).then(() => {
        let res = companyData.filter((company) => company._id !== id)
        setCompanyData(res)
        ...
        toast("error deleting Company , please try Again", {
          position: "top-right",
          autoClose: 5000,
          hideProgressBar: false,
          closeOnClick: true,
   ...
   const currentData = companyData.filter(company => company.Name?.toLowerCase().includes(searchQuery.toLowerCase())).slice(startIndex, endIndex);
  ...
  return (
    <div>
      <div className='flex justify-between my-4'>
        <h1 className='font-bold text-3xl'>List Company</h1>
        <Button className='bg-pink-600 hover:bg-pink-700 hover:text-white px-7   text-white' onClick={showModal}>
          Add Company
        </Button>
      </div>

      {companyData.length > 0 ? (<div className='flex justify-start items-center gap-4 my-3'>
          <label className='text-xs font-bold' htmlFor="CompanyName">Search company</label>
          <input
            type='text'
        ...
        <table className="w-full text-sm text-left text-gray-400">
          ...
          <tbody>
            {currentData.map((company) => (
              ...
              <td className="  pr-4 py-4">
                <button onClick={() => DeleteCompanyById(company._id)} className='bg-pink-600 rounded-md text-white px-8 py-2'>Delete</button>
              </td>
            </tr>
            ))}
          </tbody>

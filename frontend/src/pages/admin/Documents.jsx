...
import { getDocs , ArchivDoc , DesarchiveDoc } from '../../API/Req'
...
const Documents = () => {
  ...
  const HandleDeArchiveDocument = (id) => {
    try {
      DesarchiveDoc(id).then(() => {
        getDocsThenFetch()
        // action
        toast("Document Archived Successfully !", {
          position: "top-right",
   ...
   const currentData = documentData.filter(document => document.Document_number.toLowerCase().includes(searchQuery.toLowerCase())).slice(startIndex, endIndex);
  ...
  return (
    ...
    <div className='flex justify-between my-4'>
      <h1 className='font-bold text-3xl'>List Documents</h1>
      <Button className='bg-pink-600 hover:bg-pink-700 hover:text-white px-7   text-white' onClick={showModal}>
        Add Document
      </Button>
    </div>

    {documentData.length > 0 ? (<div className='flex justify-start items-center gap-4 my-3'>
        <label className='text-xs font-bold' htmlFor="CompanyName">Search Document</label>
        <input
          type='text'
          placeholder='Search Document by Number'
          value={searchQuery}
      ...
      <table className="w-full text-sm text-left text-gray-400">
        ...
        <tbody>
          {currentData.map((document) => (
            ...
            <th scope="row" className="px-6 py-4 text-xs  whitespace-nowrap text-white">
            ...
            {document.Archived ? <span className='text-xs bg-green-500 px-2 text-white rounded-full py-1.5'>True</span> : <span className='text-xs rounded-full bg-red-500 px-2 text-white py-1.5'>False</span>}
            </td>
            ...
            <td className="  pr-4 py-4">
              {document.Archived 
              ?
              <button onClick={() => HandleDeArchiveDocument(document._id)} className='bg-pink-800 rounded-md text-white px-8 py-2'>Désarchivé</button>
              :
              <button onClick={() => HandleArchiveDocument(document._id)} className='bg-pink-600 rounded-md text-white px-8 py-2'>Archivé</button>
              }
            </td>
            <td className="  pr-4 py-4">
              <button className='bg-orange-600 rounded-md text-white px-8 py-2'>Update</button>
            </td>
          </tr>
          ))}
        </tbody>
